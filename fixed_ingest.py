...
def read_pdf_fallback(path: str) -> str:
    """Fallback PDF reading using alternative methods"""
    print("🔄 Using fallback PDF reading method...")
    try:
        # Try with PyMuPDF (fitz) as fallback
        import fitz
        doc = fitz.open(path)
        text = ""
        for page in doc:
        ...
        try:
            ...
            print("❌ No PDF fallback libraries available")
            return "PDF reading failed - no fallback libraries installed"
    except Exception as e:
        print(f"❌ Fallback PDF reading failed: {e}")
